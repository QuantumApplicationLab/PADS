...
class SVG:
    def __init__(self, bbox, stream, standalone=True, prefix=None, indentation=0):
...
"""
    ...
    def circle(self, center, radius, style={}, **morestyle):
        ...
        self.element(
            'circle cx="%s" cy="%s" r="%s"' % (_coord(center.real), _coord(center.imag), _coord(radius)),
            style=style,
            **morestyle,
        )
    ...
    def rectangle(self, p, q, style={}, **morestyle):
        ...
        height = abs((p - q).imag)
        self.element(
            'rect x="%s" y="%s" width="%s" height="%s"' % (_coord(x), _coord(x), _coord(width), _coord(height)),
            style=style,
            **morestyle,
        )
    ...
    def polygon(self, points, style={}, **morestyle):
        ...
        self.element('polygon points="%s"' % pointlist, style=style, **morestyle)

    def polyline(self, points, style={}, **morestyle):
        """Polyline with corners at the given set of points"""
        pointlist = " ".join(_coord(p.real) + "," + _coord(p.imag) for p in points)
        self.element('polyline points="%s"' % pointlist, style=style, **morestyle)

    def segment(self, p, q, style={}, **morestyle):
        """Line segment from p to q"""
        self.element(
            'line x1="%s" y1="%s" x2="%s" y2="%s"' % (_coord(p.real), _coord(p.imag), _coord(q.real), _coord(q.imag)),
            style=style,
            **morestyle,
        )
    ...
    def arc(self, p, q, r, large=False, style={}, **morestyle):
        ...
        r = _coord(abs(r))
        self.element(
            'path d="M %s,%s A %s,%s 0 %s 0 %s,%s"'
            % (_coord(p.real), _coord(p.imag), r, r, large, _coord(q.real), _coord(q.imag)),
            style=style,
            **morestyle,
        )
    ...
    def text(self, label, location, style={}, **morestyle):
        ...
        self.element(
            'text x="%s" y="%s"' % (_coord(location.real), _coord(location.imag)),
            delta=1,
            unspaced=True,
            style=style,
            **morestyle,

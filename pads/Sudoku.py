...
class Sudoku:
    ...
    def place(self, digit, cell, explanation=None):
        ...
        if self.contents[cell]:
            ...
            raise BadSudoku("place(%d,%d): cell already contains %d" % (digit, cell, self.contents[cell]))
        if (1 << cell) & self.locations[digit] == 0:
            self.log(
                ["Unable to place", digit, "in", cellnames[cell], "as that digit is not available to be placed there."]
            )
    ...
    for d in digits:
        for g in groups:
            ...
            if dglocs & (dglocs - 1) == 0:
                ...
                grid.place(d, unmask[dglocs], ["It is the only cell in", g.name, "in which", d, "can be placed."])
...
enough_room = "To leave enough room for those digits, no other " "digits may be placed in those cells."
            ...
            for d in digits:
                ...
                def explain():
                    ...
                    if d not in forces:
                        ...
                        og = sqr.name
                    return ing + ["Therefore,", d, "may not be placed", "in any other cell of", og]
    ...
    for d in digits:
        ...
        if locs != mask:
            ...
            def explain():
                ...
                return (
                    ex
                    + " not part of any valid placement of this digit that covers each row, column and 3x3 square exactly once."
                )
...
def rectangle(grid):
    ...
    for r1, r2, c1, c2 in rectangles():
        ...
        if not (mask & grid.original_cells):
            ...
            for x1, perp in ((r1, (c1, c2)), (r2, (c1, c2)), (c1, (r1, r2)), (c2, (r1, r2))):
                xd = [d for d in digits if grid.locations[d] & mask & x1.mask]
                if len(xd) == 2:  # found locked pair on x1's corners
                    for x2 in perp:
                        for d in xd:
    ...
    for g in groups:
        ...
        while imp:
            ...
            def explain():
                ...
                expls[-1] += "."
                return ["In", g.name + ","] + expls + ["Placing", d, "in", namecells(mask, "or"), that]
                ...
                if (cell, d) in grid.bilocation.reachable(cell, d):
                    ...
                    def explain():
                        ...
                        if len(cycle) == 4:
                            ...
                            expl += ["cycle's remaining", len(cycle) - 3, "digits", "in the remaining"]
                            if len(cycle) == 5:
                                expl.append("cell.")
                            else:
                ...
                if cell in grid.bivalues:
                    for neighbor, nd in grid.bivalues.reachable(cell, grid.otherbv[cell, d]):
                        if d == grid.otherbv[neighbor, nd]:
                            ...
                            def explain():
                                path = grid.bivalues.shortest(cell, grid.otherbv[cell, d], neighbor, nd)
                                return [
                                    inpath,
                                    pathname(path) + ",",
                                    bivalue_explanation + ".",
...
def twosat(grid):
    ...
    for cell in range(81):
        if len(grid.choices(cell)) > 1:
            for d in grid.choices(cell):
                T[(cell, d)] = [Not((cell, e)) for e in grid.choices(cell) if d != e]
                T[Not((cell, d))] = []
                        ...
                        if len(ch) == 3:
                            ...
                            for dd in [x, y]:
                                ...
                                while other:
                                    ...
                                    T[Not((candidate, thirdchoice))].append(Not((interference, dd)))
    ...
    for name, rule, level in rules:
        if level <= 1 or not quick_and_dirty:
            ...
            if grid.progress:
                ...
                grid.log(["Ending solver iteration", grid.steps, "after successful application of the", name, "rule."])
                return True
...
def permute(grid, preserve_symmetry=True):
    ...
    for row in range(9):
        for col in range(9):
            contents[row_permutation[row] * transpose[0] + col_permutation[col] * transpose[1]] = digit_permutation[
                grid.contents[9 * row + col]
            ]
...
def svg_format(grid):
    ...
    for i in [1, 2, 4, 5, 7, 8]:
        ...
        svg.segment(pos + 2j, pos + 272j)
    svg.ungroup()
    svg.group(style={"font-family": "Times", "font-size": "24", "fill": "black", "text-anchor": "middle"})
    for row in range(9):
        for col in range(9):
            cell = row * 9 + col
            if grid.contents[cell]:
...
def all_solutions(grid, fastrules=True):
    ...
    while True:
        ...
        grid.unplace(
            d,
            1 << c,
            "The backtracking search has already tried this" " placement, and now must try the opposite decision.",
        )
...
parser.add_option(
    "-r", "--rules", dest="show_rules", action="store_true", help="show description of known solver rules and exit"
)

parser.add_option(
    "-l", "--levels", dest="show_levels", action="store_true", help="show description of difficulty levels and exit"
)

parser.add_option("-0", "--blank", dest="empty", action="store_true", help="output blank sudoku grid and exit")

parser.add_option(
    "-t", "--translate", dest="translate", action="store_true", help="translate format of input puzzle without solving"
)

parser.add_option("-p", "--permute", dest="permute", action="store_true", help="randomly rearrange the input puzzle")

parser.add_option(
    "-g", "--generate", dest="generate", action="store_true", help="generate new puzzle rather than reading from stdin"
)

parser.add_option(
    "-a", "--asymmetric", dest="asymmetric", action="store_true", help="allow asymmetry in generated puzzles"
)
...
parser.add_option("-s", "--satisfiability", dest="twosat", action="store_true", help="enable 2-satisfiability solver")

parser.add_option(
    "-b", "--backtrack", dest="backtrack", action="store_true", help="enable trial and error search for all solutions"
)

parser.add_option(
    "-v", "--verbose", dest="verbose", action="store_true", help="output description of each step in puzzle solution"
)
...
parser.add_option(
    "-2", "--output-both", dest="output_both", action="store_true", help="output both the puzzle and its solution"
)

...
def ConcaveMinima(RowIndices, ColIndices, Matrix):
    ...
    stack = []
    for r in RowIndices:
        while len(stack) >= 1 and Matrix(stack[-1], ColIndices[len(stack) - 1]) > Matrix(r, ColIndices[len(stack) - 1]):
            stack.pop()
    ...
    minima = ConcaveMinima(RowIndices, [ColIndices[i] for i in range(1, len(ColIndices), 2)], Matrix)
